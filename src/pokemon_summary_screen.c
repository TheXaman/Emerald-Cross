...
static void BufferMonTrainerMemo(void)
{
    ...
    else
    {
		...
		if (sum->metGame == VERSION_GAMECUBE && sum->fatefulEncounter)
			DynamicPlaceholderTextUtil_SetPlaceholderPtr(8, gGameNames[VERSION_GAMECUBE + 1]);
		else
			DynamicPlaceholderTextUtil_SetPlaceholderPtr(8, gGameNames[sum->metGame]);
        ...
        else if (sum->metLocation != METLOC_IN_GAME_TRADE && DidMonComeFromGBAGames())
        {
			DynamicPlaceholderTextUtil_SetPlaceholderPtr(4, metLocationString);
            if (sum->metLevel == 0)
                text = (sum->metLocation >= MAPSEC_NONE) ? gText_XNatureHatchedSomewhereAt : gText_XNatureHatchedAtYZTrade;
            else
                text = (sum->metLocation >= MAPSEC_NONE) ? gText_XNatureMetSomewhereAt : gText_XNatureMetAtYZTrade;
        }
...
static u8 WhatRegionWasMonCaughtIn(struct Pokemon *mon)
{
	...
	else if (originGame == 0 || (originGame >= 6 && originGame <= 14))
		return REGION_UNKNOWN;
	else
		return REGION_HOENN;
}
...
{
    struct PokeSummary *sum = &sMonSummaryScreen->summary;
    if (sum->metGame == VERSION_HEART_GOLD)	//CrystalDust uses HeartGold's game ID
        return TRUE;
    return FALSE;
}
